...
class Pesanan extends BaseController
{
    ...
    public function index()
    {
        ...
        return view('/dashboard/order_handler', ['data' => $data]);
    }
    ...
    public function cancelOrder()
    {
        ...
        if ($pembayaran) {
            $pembayaranModel->save([
                ...
                'status' => 'Bukti tidak valid, silahkan upload kembali',
                'bukti' => null
            ]);
            unlink('uploads/bukti_pembayaran/' . $this->request->getVar('bukti'));
            return redirect()->to(base_url('/order-handler'));
        } else {
            return redirect()->to(base_url('/order-handler'));
        }
    }
